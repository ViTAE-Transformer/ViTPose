...
def _inference_single_pose_model(model,
    ...
    for data in batch_data['img_metas'].data:
        image_meta.append(data[0])
        
    # forward the model
    with torch.no_grad():
        result = model(img=batch_data['img'].to(device), img_metas=image_meta, return_loss=False, return_heatmap=return_heatmap)
...
def _inference_single_pose_model_batch(model,
                                 imgs,
                                 bboxes,
                                 dataset='TopDownCocoDataset',
                                 dataset_info=None,
    ...
    for idx, bbox in enumerate(bboxes):
        ...
        if isinstance(imgs[idx], np.ndarray):
            data['img'] = imgs[idx]
        else:
            data['image_file'] = imgs[idx]
    ...
    image_meta = []
    # batch_data = scatter(batch_data, [device])[0]
    for data in batch_data['img_metas'].data[0]:
        image_meta.append(data)
...
def inference_top_down_pose_model_batch(model,
                                  imgs,
                                  person_results=None,
                                  bbox_thr=None,
                                  format='xywh',
    ...
    if person_results is None:
        ...
        person_results = [{'bbox': np.array([0, 0, width, height])} for _ in range(len(imgs))]

    if len(person_results) == 0:
        return pose_results, returned_outputs

    # Change for-loop preprocess each bbox to preprocess all bboxes at once.
    bboxes = np.array([box['bbox'] for box in person_results]) # (bs, 4)
    ...
    with OutputHook(model, outputs=outputs, as_tensor=False) as h:
        # poses is results['pred'] # N x 17x 3
        poses, heatmap = _inference_single_pose_model_batch(
            model,
            imgs,
            bboxes_xywh,
            dataset=dataset,
